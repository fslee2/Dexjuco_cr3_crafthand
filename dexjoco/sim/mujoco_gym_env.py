...
class MujocoGymEnv(gym.Env):
    ...
    def __init__(
        ...
        if (xml_path is None) == (model is None):
            raise ValueError("Provide exactly one of xml_path or model.")
        self._model = (
            model
            if model is not None
            else mujoco.MjModel.from_xml_path(xml_path.as_posix())  # type: ignore[union-attr]
        )
