...
class Cr3CraftWaterPlantEnv(MujocoGymEnv):
    def __init__(
        ...
        super().__init__(
            xml_path=_XML_PATH if model is None else None,
            model=model,
            seed=seed,
    ...
    def step(self, action: np.ndarray):
        ...
        # cone alpha logic
        watering_joint_0_pos = float(self._data.sensor("spray_joint_0_pos").data[0])
    ...
    def _compute_observation(self) -> dict:
        ...
        policy_hand_qpos = self._hand.policy_state(self._data)
        ...
        obs["state"] = {
            "tcp_pose": tcp_pose,
            "gripper_pose": policy_hand_qpos.astype(np.float32),
